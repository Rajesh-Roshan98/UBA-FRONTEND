    ...
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 bg-gray-100 min-h-full w-full">
        <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
          ...
          {/* --- Header --- */}
          {/* Responsive Header: Stacks vertically on mobile, row on tablet+ */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Dashboard Overview</h1>
              <p className="text-sm sm:text-base text-gray-600">System monitoring is active.</p>
            </div>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              {quickLinks.map((link, idx) => (
                <Link 
                  key={idx} 
                  to={link.path}
                  className={`group relative p-4 sm:p-6 rounded-xl border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1 hover:border-gray-300 ${idx === 0 ? 'md:col-span-2' : ''}`}
                >
                  ...
                  <p className="text-gray-500 text-xs sm:text-sm leading-relaxed">{link.desc}</p>
                </Link>
              ))}
            </div>

            {/* Right Column: Risk Score Widget */}
            <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 flex flex-col justify-between relative overflow-hidden">
              <div>
              ...
              <div className="flex flex-col items-center justify-center py-6">
                <div className="relative">
                  ...
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                    ...
                    <span className="block text-[8px] sm:text-[10px] text-gray-500 uppercase tracking-wider">{riskLabel}</span>
                  </div>
                </div>
              </div>

              <div className={`border rounded-lg p-3 flex items-start sm:items-center gap-3 ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {stats && stats.map((stat, index) => {
              ...
              return (
                <div key={index} className="bg-white rounded-xl shadow-md p-4 sm:p-5 hover:shadow-lg transition-shadow group">
                  <div className="flex items-start justify-between">
                    <div className="text-left">
                      <p className="text-gray-500 text-[10px] sm:text-xs font-medium uppercase tracking-wider mb-1">
                        {stat.label}
                      </p>
                      ...
                      {isSessionCard && subtext && (
                        <p className="text-[10px] sm:text-[11px] text-gray-500 mt-1 font-mono tracking-tight opacity-80">
                          {subtext}
                        </p>
                      )}
                    </div>
                    <div className={`p-2 sm:p-2.5 rounded-xl bg-gray-100 ${uiConfig.color} group-hover:scale-110 transition-transform`}>
