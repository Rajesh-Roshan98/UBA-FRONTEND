    ...
    <>
      ...
      >
        <div className="max-w-md w-full text-center flex flex-col items-center animate-in fade-in duration-200">
          ...
          {errorMessage && (
            <p className="text-sm sm:text-base text-red-500 mb-6 font-medium animate-in fade-in">
              {errorMessage}
            </p>
          )}

          {/* 🔥 CSS FIX: Full width on mobile, auto width on tablet+. Larger padding for thumbs. */}
          <button
            onClick={handleRetry}
            disabled={isRetrying}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3.5 sm:py-2.5 bg-gray-900 text-white text-base sm:text-sm font-medium rounded-xl sm:rounded-lg shadow-md hover:bg-gray-800 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 disabled:opacity-70 disabled:cursor-not-allowed sm:min-w-[140px]"
          >
            {isRetrying ? (
              <>
